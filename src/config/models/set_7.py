...
class PromedioAnalistas(BaseModel):
    promedio_an1: float = Field(..., description="Promedio de los porcentajes obtenidos de las replicas evaluadas en el parametro de precision intermedia para el analista 1")
    promedio_an2: float = Field(..., description="Promedio de los porcentajes obtenidos de las replicas evaluadas en el parametro de precision intermedia para el analista 2")
...
class DatosPrecisionIntermediaStrOutput(BaseModel):
    ...
    promedio_analistas: List[PromedioAnalistas] = Field(..., description="Promedio de los porcentajes obtenidos de las replicas evaluadas en el parametro de precision intermedia")
...
class ActivoPrecisionIntermediaStrOutput(BaseModel):
    ...
    rsd_an1_an2: float = Field(..., description="RSD de los porcentajes obtenidos del activo evaluado en el parametro de precision del metodo")
    diferencia_promedio_an1_an2: float = Field(..., description="Diferencia de los porcentajes obtenidos del activo evaluado en el parametro de precision del metodo")
