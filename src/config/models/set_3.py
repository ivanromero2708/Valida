...
class ParametroLinealidadExtraccion(BaseModel):
    nombre: Optional[str] = Field(..., description="Nombre del ingrediente activo con la data de validación de linealidad")
    linealidad_sistema: Optional[List[LinealidadSistemaExtraccion]] = Field(None, description="Datos de linealidad del sistema")
    rsd_factor: Optional[float] = Field(..., description="RSD de los Factores de Respuesta (Reportado como RSD Response Factor)")
    pendiente: Optional[float] = Field(..., description="Pendiente de la curva (b) (Reportado como Slope)")
    intercepto: Optional[float] = Field(..., description="Intercepto (a) (Reportado como Intercept)")
    r: Optional[float] = Field(..., description="Coeficiente de correlación (r) (Reportado como Correlation coefficient (r))")
    r2: Optional[float] = Field(..., description="Coeficiente de Determinación (r2) (Reportado como Determination coefficient (r2))")
    porcentaje_intercepto: Optional[float] = Field(..., description="Intercepto como porcentaje de 0 al 100% (Reportado como Intercepto as percentage of Y at 100%)")
    cumple_global: Optional[Literal["Cumple", "No Cumple"]] = Field(None, description="Cumple con el criterio global")
